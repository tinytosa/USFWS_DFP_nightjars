...
## Determining whether we can keep existing routes
...
for(st in unique(AF_states$ST))
{
  ...
  random.st.sf <- st_as_sf(to_combine[to_combine$ST == st,], coords=c("X","Y"), crs=crs(BCR))
  existing.sf <- st_as_sf(to_combine[to_combine$ST == st,], coords=c("Longitude","Latitude"), crs=crs(BCR))
  ...
  ggplot() + 
    ...
    geom_sf(data=all.random.sf, aes(col=selected), pch=17, size=3) +
    ...
    geom_text_repel(data=existing.sf, aes(label=st_route, geometry=geometry, col=selected), 
                  stat = "sf_coordinates") +
