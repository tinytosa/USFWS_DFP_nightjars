...
#can you set so points have to be a certain distance away from each other???
...
dist.long <- pivot_longer(dist.routes, values_to="dist", cols='FL_0001..1':'CT_NB86..11')
...
# write.csv(summary.need.have, file="../Data_processed/random_pts_area3k_vs_existingroutes.csv", row.names=F)
...
for(st in unique(AF_states$ST))
{
  ...
  existing.sf <- st_as_sf(to_combine[to_combine$ST == st,], coords=c("Longitude","Latitude"), crs=crs(BCR.ST))
  # all.random.sf <- random.sf[random.sf$ST == st &
  #                              !random.sf$route_name %in% random.st.sf$route_name,]

  
  random.st.sf <- st_as_sf(to_combine, coords=c("X","Y"), crs=crs(BCR.ST))
  # existing.sf <- st_as_sf(to_combine, coords=c("Longitude","Latitude"), crs=crs(BCR))

  all.random.sf <- random.sf[random.sf$ST == st &
                               !random.sf$route_name %in% to_combine[to_combine$ST == st,]$route_name,]
  ...
  plot + 
    geom_sf(data=all.random.sf[all.random.sf$selected == 1,], aes(col=alternate), pch=17, size=3) + #triangle for random points without existing nearby
    ...
    geom_text_repel(data=existing.sf[existing.sf$selected == 1,],
                    aes(label=st_route, geometry=geometry, col=alternate), stat = "sf_coordinates", col="steelblue") +
    coord_sf(xlim=c(ex@xmin, ex@xmax), ylim=c(ex@ymin, ex@ymax))
  ...
  plot + 
    ...
    geom_text_repel(data=existing.sf, aes(label=st_route, geometry=geometry, col=alternate), stat = "sf_coordinates") +
    coord_sf(xlim=c(ex@xmin, ex@xmax), ylim=c(ex@ymin, ex@ymax))
