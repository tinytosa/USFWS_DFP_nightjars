...
#1. add nsn.obs to nsn.sampling
  ...
  nsn.sampling[!nsn.sampling$id %in% nsn.obs$survey_stop_id,] #28,701 rows!!!
  nrow(nsn.sampling[!nsn.sampling$id %in% nsn.obs$survey_stop_id,])

master <- merge(nsn.obs, nsn.sampling, by.x="survey_stop_id", by.y="id", all=T, suffixes=c(".obs",".sampling")) #68,359 rows (nrow(nsn.obs + 28,701))
