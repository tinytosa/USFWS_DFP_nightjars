...
nsn.obs <- read.csv("../Data_raw/Copy of NSN Data/NSN Data (March 28 2023) - Point Count Observations.csv") #39,658 observations
  ...
  unique(nsn.obs[duplicated(nsn.obs[,c("survey_stop_id","species_id")]),]$survey_stop_id) #hmmmmm...9640 duplicated observations for sampling occasions!
...
master <- merge(nsn.obs, nsn.sampling, by.x="survey_stop_id", by.y="id", all=T, suffixes=c(".obs",".sampling")) #68,359 rows

#2. add nsn.time to master
master <- merge(master, nsn.time, by.x="survey_id", by.y="id", all=T, suffixes=c("",".time")) #68,359 rows
...
master <- merge(master, nsn, by.x=c("route_id","stop_mit"), by.y=c("Routeid","Stop"), all=T, suffixes=c("",".route")) #71,024 rows

#4. add rt.id to master
nrow(rt.id[!(rt.id$corrected %in% master$route_id),]) #420 rows not in master already
master <- merge(master, rt.id, by.x="route_id", by.y="corrected", all=T, suffixes=c("",".rtid")) #71,444 rows

#5 add nsn.sp to master
master <- merge(master, nsn.sp[,c("id","common_name","scientific_name","code")], by.x="species_id", by.y="id", all.x=T)
...
unique(me.obs[!me.obs$Point.Count.. %in%
                  unique(me.sites$Point.Count..),]$Point.Count..)
#yes all obs in me.obs have site data associated with them

me <- merge(me.sites, me.nightjar.obs, by.x=c("Sampling.Event.UNIQUE.ID", "Point.Count..","YEAR"), by.y=c("Sampling.Event.ID","Point.Count..","YEAR"), all=T)

me[is.na(me$Species),]$Species <- "NONE"

table(me[,c("Species","YEAR")])

names(me) <- gsub(names(me), pattern="Minute", replacement="min")
...
write.table(non.nsn.master,
            file="../Data_processed/master_nonnsn_2023-06_21.txt", sep=",", row.names=F)
```
