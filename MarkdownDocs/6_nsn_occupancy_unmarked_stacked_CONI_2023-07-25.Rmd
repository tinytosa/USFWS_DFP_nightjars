...
#y=1/(y=0 + y=1)
2286/(74718 + 2286) #only 3.0% of occasions where CWWI were detected (with all NA data removed) for the detection only model
2371/(65508 + 2371) #only 3.5% of occasions where CWWI were detected for the occupancy only model
...
Sites with at least one detection: 777 
...
plogis(-5.8) #detection -> 0.3% detection
plogis(-1.44)  #occupancy -> 19.2% occupancy
...
p.month <- occu(~start_month + (1|st_id) + (1|state) ~1, data=umf)
...
p.fl <- fitList(null, null.r, p.min, #per minute within a survey
                ...
                p.day, p.day2, p.month, p.time, p.time2, p.timecat, p.hour) #, #timing of survey p.month2, 
                # p.global) 
modSel(p.fl)
#           nPars      AIC   delta    AICwt cumltvWt
# p.hour       19 10950.23    0.00  1.0e+00     1.00 #too many categories when factor
# p.timecat     5 11025.66   75.44  4.2e-17     1.00
# p.day2        4 11026.26   76.03  3.1e-17     1.00
# p.noise       3 11030.32   80.09  4.1e-18     1.00
# p.time2       4 11033.42   83.20  8.6e-19     1.00 #
# p.month       5 11034.94   84.71  4.0e-19     1.00 #
# p.moon        3 11035.39   85.16  3.2e-19     1.00
# p.time        3 11038.21   87.98  7.9e-20     1.00 #

# null.r        2 11038.70   88.47  6.1e-20     1.00
# p.day         3 11039.64   89.41  3.8e-20     1.00
# p.min         3 11039.86   89.63  3.4e-20     1.00
# p.sky         3 11040.53   90.30  2.5e-20     1.00
# null          2 12293.68 1343.46 1.9e-292     1.00

p.global <- occu(~start_time_cat + scale(noise) + scale(start_date_j) + start_date_j2 + moon +
                   (1|st_id) + (1|state) ~1, data=umf)
p.global2 <- occu(~start_time_cat + scale(noise) + scale(start_date_j) + I(scale(start_date_j) * scale(start_date_j)) + moon +
                   (1|st_id) + (1|state) ~1, data=umf)
...
ggsave(c.plot, filename="../Figures/occupancy/CONI_stacked_occupancy_det_coefficients.tiff")
...
plot.marginal(sp="CONI", p.global2, "start_date_j", "Julian day", type="det") #need to plot separately with p.global2 since have scale within I()

#plot separately for categorical and obsCovs
#for categorical
sp <- "CONI"; par <- "start_time_cat"; par.name <- "start time"; type <- "det"
...
Maximum number of observations per site: 6 
Mean number of observations per site: 6 
Sites with at least one detection: 810 
...
modSel(o.fl)
#         nPars      AIC delta   AICwt cumltvWt
# o.lat2      4 11066.37  0.00 1.0e+00     1.00
# o.long2     4 11121.81 55.43 9.2e-13     1.00
# o.lat       3 11128.80 62.42 2.8e-14     1.00
# o.long      3 11141.59 75.22 4.6e-17     1.00

o.fl <- fitList(null, o.bcr, o.state, o.year,
                o.lat, o.lat2, o.long, o.long2)
modSel(o.fl)
#         nPars      AIC  delta   AICwt cumltvWt
# o.state    11 11032.45   0.00 1.0e+00     1.00
# o.bcr       8 11076.90  44.46 2.2e-10     1.00
# o.year     17 11120.98  88.53 6.0e-20     1.00
# null        2 11139.61 107.16 5.4e-24     1.00

#         nPars      AIC  delta   AICwt cumltvWt
# o.state    11 11032.45   0.00 1.0e+00     1.00
# o.lat2      4 11066.37  33.93 4.3e-08     1.00
# o.bcr       8 11076.90  44.46 2.2e-10     1.00
# o.year     17 11120.98  88.53 6.0e-20     1.00
# o.long2     4 11121.81  89.36 3.9e-20     1.00
# o.lat       3 11128.80  96.35 1.2e-21     1.00

# null        2 11139.61 107.16 5.4e-24     1.00
# o.long      3 11141.59 109.15 2.0e-24     1.00

o.state.year <- occu(~1 ~ state + year + (1|st_id), data=umf)
o.bcr.year <- occu(~1 ~ BCR + year + (1|st_id), data=umf)

# o.state.bcr.year <- occu(~1 ~ state + BCR + year + (1|st_id), data=umf) #can't do this
