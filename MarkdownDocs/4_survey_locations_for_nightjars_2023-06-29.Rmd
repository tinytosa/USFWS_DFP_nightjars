...
dist.long <- pivot_longer(dist.routes, values_to="dist", cols='FL_0001..1':'CT_WH85..9')  # CT_NB86..11
...
# write.csv(summary.need.have, file="../Data_processed/random_pts_area3k_vs_existingroutes.csv", row.names=F) #this isn't quite right since includes routes from a different state
...
state_table <- merge(to_combine[,c("existing_rt_stop","existing_route","existing_route_name","state.existing","stop",
...
names(state_table) <- c("random_route_name","selected","existing_rt_stop","existing_route","existing_route_name","state.existing","stop",
...
for(st in unique(AF_states$ST))
{
  ...
  existing.sf <- st_as_sf(to_combine[to_combine$state.random == st,], coords=c("Longitude.existing","Latitude.existing"), crs=crs(BCR.ST))
  # all.random.sf <- random.sf[random.sf$ST == st &
  #                              !random.sf$route_name %in% random.st.sf$route_name,]

  
  random.st.sf <- st_as_sf(to_combine, coords=c("X.random","Y.random"), crs=crs(BCR.ST))
